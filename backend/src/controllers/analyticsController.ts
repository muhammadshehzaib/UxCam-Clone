...
export async function getFeedbackSubmissions(req: ProjectRequest, res: Response): Promise<void> {
  const days = Math.min(90, Math.max(1, parseInt(req.query.days as string) || 30));
  try {
    const data = await analyticsService.getFeedbackSubmissions(req.project!.id, days);
    res.json({ data });
  } catch (err) {
    console.error('getFeedbackSubmissions error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
}
